...
const TYPES  = ['CL — Chain Links','HK — Hooks','FR — Fibre Ropes','SH — Shackles','SWR — Steel Wire Ropes','OTH — Other'];
...
function renderItems() {
  ...
  if(!items.length) {
    tbody.innerHTML='<tr class="empty-row"><td colspan="6">No items added. Click "+ Add item" to begin.</td></tr>';
    return;
  }
  items.forEach((item,idx) => {
    ...
    tr.innerHTML=`
      <td><input type="text" value="${esc(item.id)}" placeholder="e.g. LE-01" onchange="data[${currentMonth}].items[${idx}].id=this.value"></td>
      <td><input type="text" value="${esc(item.location)}" placeholder="Location" onchange="data[${currentMonth}].items[${idx}].location=this.value"></td>
      <td><select onchange="data[${currentMonth}].items[${idx}].type=this.value">${typeOpts}</select></td>
      ...
      <td><input type="text" value="${esc(item.deviation)}" placeholder="e.g. 1.3" onchange="data[${currentMonth}].items[${idx}].deviation=this.value"></td>
...
function renderSignoff() {
  ...
  if(so){
    ...
    d.innerHTML=`<span class="signed-badge">&#10003; Signed off</span>
      <span style="font-size:12px;color:var(--text-secondary);">Date: <strong>${so.date}</strong> &nbsp;|&nbsp; Inspector: <strong>${so.inspector}</strong> &nbsp;|&nbsp; Spot-check: <strong>${so.audit||'—'}</strong></span>
...
function signOff() {
  ...
  ['so-date','so-inspector','so-audit'].forEach(id=>document.getElementById(id).value='');
  renderSignoff();
}
